...
// 🌐 HTTP version of createCheckoutSession with CORS support for web compatibility
exports.createCheckoutSessionHttp = functions.https.onRequest(async (req, res) => {
    // Set CORS headers to allow requests from localhost and production
    ...
    try {
        ...
        if (decodedToken.uid !== data.userId) {
            res.status(403).json({ error: 'Forbidden - can only create subscription for authenticated user' });
            return;
        }
        // Validate coupon if provided
        let validatedCoupon = undefined;
        ...
        functions.logger.info('🔒 Creating secure checkout session (HTTP)', {
            userId: data.userId,
            userType: data.userType,
            priceId: data.priceId,
            environment: data.environment,
        ...
        // Create the checkout session
        const session = await stripe.checkout.sessions.create(sessionConfig);
        functions.logger.info('✅ Checkout session created (HTTP)', {
            sessionId: session.id,
            url: session.url,
        });
        res.status(200).json({
        ...
        functions.logger.error('❌ Error creating checkout session (HTTP)', error);
