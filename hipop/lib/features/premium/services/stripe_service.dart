...
class StripeService {
  ...
  static Future<String> createCheckoutSession({
    ...
    try {
      ...
      try {
        ...
        debugPrint('✅ Secure checkout session created via callable: $checkoutUrl');
        return checkoutUrl;
      } catch (callableError) {
        debugPrint('⚠️ Callable function failed, trying HTTP fallback: $callableError');
        
        // If callable fails (likely CORS on localhost), try HTTP endpoint
        if (kIsWeb) {
          return await _createCheckoutSessionHttp(requestData);
        } else {
          rethrow;
        }
      }
  ...
  /// HTTP fallback for createCheckoutSession when callable functions have CORS issues
  static Future<String> _createCheckoutSessionHttp(Map<String, dynamic> requestData) async {
    debugPrint('🌐 Creating checkout session via HTTP endpoint');
    ...
    try {
      ...
      // Make HTTP request to the new endpoint
      final response = await http.post(
        Uri.parse('https://us-central1-hipop-markets.cloudfunctions.net/createCheckoutSessionHttp'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (checkoutUrl == null || checkoutUrl.isEmpty) {
          throw Exception('No checkout URL returned from HTTP endpoint');
        }
        
        debugPrint('✅ Secure checkout session created via HTTP: $checkoutUrl');
        return checkoutUrl;
      } else {
        final errorData = json.decode(response.body);
        throw Exception('HTTP request failed: ${errorData['error'] ?? 'Unknown error'}');
      }
    } catch (e) {
