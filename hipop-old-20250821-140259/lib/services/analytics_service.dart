...
class AnalyticsService {
  ...
  static Future<AnalyticsSummary> getAnalyticsSummary(
    ...
    try {
      ...
      // Get real user engagement data from RealTimeAnalyticsService
      final engagementData = await _getRealEngagementMetrics(marketId, timeRange);
      ...
      return AnalyticsSummary(
        ...
        totalEvents: 0,
        totalRecipes: recipeMetrics['total'] ?? 0,
        totalViews: engagementData['totalViews'] ?? 0, // Now using real view tracking
        growthRate: await _calculateRealGrowthRate(marketId, timeRange),
        ...
        dailyData: await _getRealDailyAnalyticsData(marketId, timeRange),
      );
    } catch (e) {
  ...
  /// Get real engagement metrics from analytics events
  static Future<Map<String, dynamic>> _getRealEngagementMetrics(
    String marketId,
    AnalyticsTimeRange timeRange,
  ) async {
    try {
      ...
      // Use RealTimeAnalyticsService to get real metrics
      ...
      return {
        'totalViews': eventsSummary['pageViews'] ?? 0,
        'uniqueVisitors': eventsSummary['uniqueUsers'] ?? 0,
        'totalSearches': eventsSummary['searches'] ?? 0,
        'averageSessionDuration': 0.0, // Will be calculated from session data
      };
    } catch (e) {
      debugPrint('Error getting real engagement metrics: $e');
      return {
        'totalViews': 0,
        'uniqueVisitors': 0,
        'totalSearches': 0,
  ...
  /// Calculate real growth rate based on historical analytics data
  static Future<double> _calculateRealGrowthRate(
    String marketId,
    AnalyticsTimeRange timeRange,
  ) async {
    try {
      final endDate = DateTime.now();
      final currentPeriodStart = endDate.subtract(Duration(days: timeRange.days));
      final previousPeriodStart = currentPeriodStart.subtract(Duration(days: timeRange.days));
      
      // Get current period metrics
      final currentMetrics = await RealTimeAnalyticsService.getEventsSummary(
        marketId: marketId,
      ...
      final previousMetrics = await RealTimeAnalyticsService.getEventsSummary(
        marketId: marketId,
        startDate: previousPeriodStart,
        endDate: currentPeriodStart,
      );
      
      final currentViews = currentMetrics['totalEvents'] ?? 0;
      final previousViews = previousMetrics['totalEvents'] ?? 0;
      
      if (previousViews == 0) {
        return currentViews > 0 ? 100.0 : 0.0;
      }
      
      final growthRate = ((currentViews - previousViews) / previousViews) * 100;
      return growthRate;
    } catch (e) {
      debugPrint('Error calculating real growth rate: $e');
      return 0.0;
    }
  }

  /// Get real daily analytics data for charts
  static Future<List<MarketAnalytics>> _getRealDailyAnalyticsData(
    String marketId,
    AnalyticsTimeRange timeRange,
  ) async {
    try {
      ...
      final dailyData = <MarketAnalytics>[];
      
      // Generate daily analytics for each day in the range
      for (int i = 0; i < timeRange.days; i++) {
        final dayStart = startDate.add(Duration(days: i));
        final dayEnd = dayStart.add(const Duration(days: 1));
        
        // Get real events for this day
        final dayEvents = await RealTimeAnalyticsService.getEventsSummary(
          marketId: marketId,
          startDate: dayStart,
          endDate: dayEnd,
        );
        
        // Get vendor and recipe metrics for this day (simplified)
        final vendorMetrics = await _getVendorMetrics(marketId);
        final recipeMetrics = await _getRecipeMetrics(marketId);
        final favoritesMetrics = await _getFavoritesMetrics(marketId);
        ...
        final analytics = MarketAnalytics(
          marketId: marketId,
          organizerId: '', // Will be filled when available
          date: dayStart,
          totalVendors: vendorMetrics['total'] ?? 0,
          activeVendors: vendorMetrics['active'] ?? 0,
          totalRecipes: recipeMetrics['total'] ?? 0,
          totalViews: dayEvents['pageViews'] ?? 0,
          uniqueVisitors: dayEvents['uniqueUsers'] ?? 0,
          totalSearches: dayEvents['searches'] ?? 0,
          totalMarketFavorites: favoritesMetrics['totalMarketFavorites'] ?? 0,
          totalVendorFavorites: favoritesMetrics['totalVendorFavorites'] ?? 0,
        );
        
        dailyData.add(analytics);
      }
      
      return dailyData;
    } catch (e) {
      debugPrint('Error getting real daily analytics data: $e');
      return [];
    }
  }
}