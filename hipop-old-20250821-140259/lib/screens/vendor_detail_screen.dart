...
class _VendorDetailScreenState extends State<VendorDetailScreen>
  ...
  void _onTabChanged() async {
    if (!_tabController.indexIsChanging || _vendor == null) return;
    
    try {
      final tabNames = ['about', 'recipes', 'gallery', 'reviews'];
      ...
      await RealTimeAnalyticsService.trackEvent(
        ...
        screenName: 'vendor_detail_screen',
        vendorId: _vendor!.id,
      );
    } catch (e) {
      debugPrint('Error tracking vendor tab change: $e');
    }
  }
  ...
  void _trackVendorView(ManagedVendor vendor) async {
    try {
      // Track page view
      await RealTimeAnalyticsService.trackPageView(
        'vendor_detail_screen',
      );
      
      // Track vendor-specific view
      await RealTimeAnalyticsService.trackVendorInteraction(
        ...
        additionalData: {
          ...
          'market_id': vendor.marketId,
          'screen_entry_time': DateTime.now().toIso8601String(),
        },
      );
    } catch (e) {
      debugPrint('Error tracking vendor view: $e');
    }
  }
  ...
  Future<void> _trackVendorContactInteraction(String url) async {
    ...
    try {
      ...
      await RealTimeAnalyticsService.trackVendorInteraction(
        ...
        _vendor!.id,
        additionalData: {
          'vendor_business_name': _vendor!.businessName,
          'vendor_name': _vendor!.vendorName ?? _vendor!.businessName,
          'contact_url': url,
          'market_id': _vendor!.marketId,
          'interaction_time': DateTime.now().toIso8601String(),
        },
      );
    } catch (e) {
      debugPrint('Error tracking vendor contact interaction: $e');
    }
  }

  Future<void> _launchUrl(String url) async {
    // Track vendor contact interactions
    await _trackVendorContactInteraction(url);
